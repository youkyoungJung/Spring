			...
			<table>
				...
				<c:if test="${not empty productList}">
					...
					<tr class="list">
						...
						<td class="pButton">
							<a class="btn btn-outline-primary buy" id="buttons" href='<c:url value='/edit/item'>
		            				<c:param name="item_id" value="${product.item_id}"/></c:url>'>
		            			수정</a>
							<a class="btn delete" id="buttons"  href='<c:url value='/delete/item'>
	            				<c:param name="item_id" value="${product.item_id}"/></c:url>' onclick = "return confirm('정말 삭제하시겠습니까?')" >
	            			삭제</a> 
	            			<a class="btn delete" id="buttons" href='<c:url value='/update/selled'>
	            				<c:param name="item_id" value="${product.item_id}"/></c:url>' onclick = "return confirm('판매 완료되면 수정이 불가합니다. 판매 완료 하시겠습니까?')">
	            			판매완료</a> 
						</td>
						</c:if>
