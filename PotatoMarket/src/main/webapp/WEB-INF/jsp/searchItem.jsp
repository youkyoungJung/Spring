		...
		<h2>
			...
			<c:if test="${isLogin}">
				<a class="btn pull-right" style="margin-right: 30px;" id="buttons"
					href="<c:url value='/add/item'>
            <c:param name='memberId' value='${member_id}'/>
            <c:param name='item_id' value='0'/></c:url>">상품
					등록</a>
			</c:if>
		</h2>
		<c:choose>
			...
			<c:if test="${product.confirm eq 0 }">
	            ...
	            <c:param name="item_id" value="${product.item_id }"/></c:url>'><c:out
						...
						<div class="card-footer">
							<small class="text-muted"> 등록일 : <c:out
									value="${product.item_date }" />
							</small>
						</div>
					</div>
				</div>
			...
			<c:if test="${product.confirm eq 1 }">
				<div class="col list-card close">
					<div class="card h-100">
						<a class="go"
							href='<c:url value="/detail/item">
	            <c:param name="item_id" value="${product.item_id }"/></c:url>'><img
							src="/images/${product.fileName}" class="card-img-top thumbnail"></a>
						<div class="card-body">
							<h5 class="card-title">
								<a class="go"
									href='<c:url value="/detail/item">
